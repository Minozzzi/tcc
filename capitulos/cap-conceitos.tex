...
O método algébrico, viabiliza a fusão de dois ou mais componentes e envolve a formulação de um sistema de equações simultâneas, em que as incógnitas correspondem aos componentes a serem incorporados na ração. A complexidade desse método aumenta de maneira gradual à medida que se incluem um maior número de componentes e nutrientes no cálculo.
...
Portanto, a ração é constituída de 16,55\% de farelo de algodão e 83,45\% de grão de milho.
...
Dessa forma, para 37,82 kg da mistura, é necessário utilizar 29,64 kg de milho e 8,18 kg de farelo de soja. Portanto, para 100 kg da mistura, são necessários 78,37 kg de milho:
\begin{equation}
 \label{eq:pearson3}
 (29,64 / 37,82)*100 
\end{equation}
...
\section{Arquitetura de Software e Clean Architecture}\label{sec:arquiteturaSoftware}

A arquitetura de software é a estrutura fundamental de um sistema, composta por seus componentes, propriedades externas e os relacionamentos entre eles. Segundo \citeonline{Martin:2017:CleanArchitecture}, uma boa arquitetura deve permitir que o sistema seja fácil de entender, desenvolver, manter e implantar. O objetivo principal é minimizar o custo humano por unidade de funcionalidade ao longo da vida útil do software.

A \textit{Clean Architecture} (Arquitetura Limpa), proposta por Robert C. Martin, foca na separação de preocupações através do encapsulamento da lógica de negócio em camadas independentes de frameworks, interfaces de usuário ou bancos de dados. Esta abordagem garante que o núcleo do software permaneça testável e resiliente a mudanças tecnológicas externas.

As camadas fundamentais geralmente incluem:
\begin{itemize}
    \item \textbf{Entidades (Domain):} Contêm as regras de negócio cruciais e objetos de domínio. São o ponto mais interno e estável da arquitetura.
    \item \textbf{Casos de Uso (Data/Use Cases):} Orquestram o fluxo de dados de e para as entidades, implementando as regras de negócio específicas da aplicação.
    \item \textbf{Adaptadores de Interface (Infra/Gateways):} Convertem dados no formato mais conveniente para os casos de uso e entidades (ex: conversores de \gls{API} \gls{REST}).
    \item \textbf{Frameworks e Drivers (Presentation/UI):} Camada na qual residem ferramentas como React e detalhes de interface.
\end{itemize}

Ao adotar esses princípios, o desenvolvimento do \textit{idr-web} busca garantir que a lógica de manejo e balanceamento nutricional esteja isolada das complexidades da interface visual, permitindo evoluções modulares e manutenção simplificada.

\section{Gerenciamento de Estado e TanStack Query}\label{sec:gerenciamentoEstado}

Em aplicações \textit{frontend} modernas, o gerenciamento de estado é o processo de controlar como os dados mudam ao longo do tempo e como essas mudanças são refletidas na \gls{UI}. O estado pode ser dividido em estado local (\gls{UI}) e estado do servidor (dados remotos).

A biblioteca TanStack Query (anteriormente React Query) destaca-se por simplificar a sincronização entre o cliente e o servidor. Ela gerencia automaticamente tarefas complexas como \textit{caching}, invalidação de consultas, atualizações em segundo plano e estados de carregamento ou erro. Ao tratar o estado do servidor como algo distinto do estado da interface, a biblioteca reduz a necessidade de códigos redundantes e melhora a experiência do usuário através de respostas instantâneas e dados sempre atualizados \cite{Dorfmeister:2022:ReactQueryGuide}.
