...
O acesso ao sistema inicia-se pela tela de autenticação, exibida na \autoref{fig:sys-login}, nela o técnico informa suas credenciais. Esta interface segue a identidade visual do \gls{IDR-PR}, garantindo que cada profissional acesse exclusivamente as propriedades sob sua responsabilidade técnica.
...
Para navegar nos detalhes da propriedade, o técnico pode clicar na linha da propriedade que deseja acessar. Ao clicar, o sistema carrega o painel central, ilustrado na \autoref{fig:sys-propriedade-detalhes}, no qual o cabeçalho exibe dinamicamente o nome da fazenda e do produtor. A interface é organizada em abas (Dados da Terra, Animais, Cultivos, etc.), permitindo que o técnico gerencie submódulos como máquinas e forrageiras sem perder o contexto da unidade produtiva.
...
Para a expansão da base de dados, o sistema oferece um formulário de cadastro de propriedades estruturado em abas, como pode ser visto na \autoref{fig:sys-propriedade-cadastro}, em que informações de colaboradores e localização geográfica são coletadas de forma segregada, garantindo uma interface limpa e organizada.
...
Por meio do módulo de propriedades é possível acessar o módulo com todos os animais dessa propriedade. A gestão do rebanho pode ser visualizada na \autoref{fig:sys-animais}. Optou-se por apresentar apenas o módulo de animais, excluindo-se os módulos de Dados da Terra e Cultivos por adotarem o mesmo estilo de implementação.
...
A \autoref{fig:sys-animal-submodulo} ilustra o submódulo de inseminações, que atua como um exemplo prático das diversas categorias de registros disponíveis. Neste ambiente, é possível cadastrar e consultar cada tentativa de reprodução e a situação de prenhez, mantendo a rastreabilidade completa para o acompanhamento do animal, tanto pelo técnico do \gls{IDR-PR} quanto pelo produtor.
...
A arquitetura dessa interface de detalhes consolida o conceito de "prontuário único" do animal, evitando que o técnico precise navegar por diferentes páginas para entender o histórico de saúde e produção. Todos os submódulos comportam-se de maneira padronizada, integrados ao mesmo conjunto de dados mantendo a integridade de informações de toda a propriedade, o que reforça a coesão do sistema perante o usuário final.

\clearpage

\subsection{Balanceamento Nutricional de Precisão}\label{subsec:apresentacaoNutricao}

A funcionalidade mais complexa do sistema é o balanceamento nutricional. A interface de Novo Balanceamento organiza-se em duas abas complementares (Visão Geral e Nutrição), sob um cabeçalho persistente que atua como suporte à navegação entre os animais da propriedade, como exibido na \autoref{fig:sys-balanceamento-visao-geral-header}. Esta funcionalidade permite que o técnico alterne sequencialmente entre os animais do lote (\textit{Animal X de N}) de forma global, mantendo o contexto de navegação independentemente da sub-aba selecionada, o que garante maior agilidade operacional.
...
Na aba de Visão Geral, abaixo da navegação global, encontra-se o bloco de \textbf{Resumo Nutricional}, ilustrado na \autoref{fig:sys-balanceamento-visao-geral}. Este bloco exibe indicadores fundamentais do processamento da dieta, como o percentual de \gls{EE} da ração, o total de \gls{MS} consumida e as relações percentuais entre volumoso e concentrado. O sistema também calcula e apresenta a relação entre \gls{PDR} e \gls{NDT}, bem como o nível de \gls{CNF}, conforme a \seqref{eq:algebrico3} apresentada na \autoref{sec:alimentacaoBalanceadaGadoLeiteiro}. Esta visão garante uma análise resumida do animal em questão do rebanho.
...
Em resumo, as interfaces de balanceamento nutricional serão de grande importância para os técnicos do \gls{IDR-PR}, pois concentram toda a informação nutricional relacionada ao rebanho leiteiro das propriedades gerenciadas. A integração entre a visualização resumida do animal e o suporte visual dinâmico durante a formulação da ração permite que o técnico execute ajustes precisos com facilidade, refletindo diretamente na eficiência produtiva e na saúde do rebanho monitorado pelo \gls{IDR-PR}.

\section{Implementação do sistema}\label{sec:implementacaoSistema}

A implementação do cliente \textit{web} teve como principal característica o uso de padrões arquiteturais que favorecem o crescimento sustentável do software. Adotou-se o paradigma da programação reativa com a biblioteca React, utilizando TypeScript para garantir a segurança de tipos em toda a aplicação. A organização do código baseia-se em uma adaptação da \textit{Clean Architecture} dividida em diretórios independentes, focada na separação de responsabilidades. Para suportar esta interface gráfica, foram criados componentes visuais genéricos como: \textit{alert-dialog, avatar, badge, breadcrumb, button, calendar, card, combobox, command, data-table, date-picker, dialog, dropdown, dropzone, form, header, input, label, loading, pagination, popover, scroll-area, select, sheet, sidebar, table, tabs, textarea, tooltip} e \textit{wave}. Todos os componentes criados podem ser visualizados nas Figuras apresentadas na seção \autoref{sec:apresentacaoSistema}.
...
Para conferir realismo ao ambiente de desenvolvimento, o sistema utiliza o \gls{MSW}\footnote{Mock Service Worker (MSW) é uma biblioteca de simulação de \gls{API} que permite interceptar requisições em nível de rede para simular respostas.}. Além de interceptar as chamadas, implementou-se uma lógica de \textbf{simulação de atraso (\textit{delay})} nas respostas, apresentado no \autoref{lst:msw-delay}, forçando a interface a lidar com estados de carregamento de forma realista.

\begin{center}
\captionof{sourcecode}{\label{lst:msw-delay}Simulação de Delay Realista no \gls{MSW}}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
...
Também foi desenvolvida uma lógica de \textbf{autenticação falsa (\textit{fake auth})} no \textit{middleware} do \gls{MSW}, exibido no \autoref{lst:msw-auth}, garantindo que as rotas protegidas e os cabeçalhos de autorização fossem validados antes de retornar os dados simulados.

\begin{center}
\captionof{sourcecode}{\label{lst:msw-auth}Middleware de Autenticação Fake no Mock}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
...
\begin{center}
\captionof{sourcecode}{\label{lst:folder-structure}Estrutura de Pastas Padronizada (Módulo Animais)}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[]
...
A camada de \textbf{Domain} é o ponto de partida, em que se definem as entidades e os contratos (casos de uso). Esta camada é dividida em modelos de dados e definições de casos de uso, ilustrados, respectivamente, no \autoref{lst:animal-models} e no \autoref{lst:animal-use-cases}, garantindo que a regra de negócio seja expressa de forma pura, sem dependências de infraestrutura.

\begin{center}
\captionof{sourcecode}{\label{lst:animal-models}Modelos de Dados do Domínio}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
...
\begin{center}
\captionof{sourcecode}{\label{lst:animal-use-cases}Contratos de Casos de Uso (\gls{CRUD}) no Domínio}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
...
\begin{center}
\captionof{sourcecode}{\label{lst:animal-data}Implementações da Camada Data (\gls{CRUD})}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
...
A camada de \textbf{Mocks} é fundamental para melhorar a experiência no desenvolvimento. Ela contém os \textit{handlers} do \gls{MSW} que interceptam especificamente as rotas deste módulo, conforme o \autoref{lst:animal-mock}. Isso permite simular o comportamento completo da \gls{API} oficial, mantendo um estado interno persistente durante a sessão para permitir testes reais de criação, edição e exclusão sem depender do \textit{backend}.

\begin{center}
\captionof{sourcecode}{\label{lst:animal-mock}Handlers \gls{MSW} para Simulação de \gls{API}}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
...
O fluxo de dados se inicia pelo \textbf{Contexto}, ilustrado no \autoref{lst:animal-context}, o qual centraliza informações como o identificador da propriedade e o animal selecionado, provendo funções de controle para abertura de formulários e diálogos, eliminando a necessidade de repasse manual de propriedades (\textit{prop drilling}).

\begin{center}
\captionof{sourcecode}{\label{lst:animal-context}Gerenciamento de Estado no Context \gls{API}}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
...
Os \textbf{Hooks} atuam como pontes entre os casos de uso e os componentes. O \texttt{useAnimalsQuery}, exibido no \autoref{lst:animal-hook}, encapsula a lógica de busca real, delegando ao \textbf{TanStack Query} o gerenciamento do estado assíncrono, cache e revalidação. O uso do \texttt{useEffect} acoplado à biblioteca \texttt{react-hot-toast} gerencia o \textit{feedback} visual de erros para o usuário.

\begin{center}
\captionof{sourcecode}{\label{lst:animal-hook}Hook de Integração com TanStack Query}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
...
Na subpasta \textbf{Components}, os elementos visuais focam apenas em renderização. O \texttt{AnimalDataTable}, apresentado no \autoref{lst:animal-table}, abstrai a complexidade da listagem, consumindo os estados de paginação e ordenação do seu próprio gancho e renderizando os dados através de um componente de tabela genérico construído na camada \texttt{core}.

\begin{center}
\captionof{sourcecode}{\label{lst:animal-table}Componente AnimalDataTable Focado em Renderização}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
...
As definições de tipos para filtros e ordenação, apresentadas no \autoref{lst:animal-types}, e as validações de formulário via Zod, ilustradas no \autoref{lst:animal-validations}, garantem a integridade dos dados e a segurança de tipos em toda a camada visual. Sempre que uma tela necessitar filtros e ordenação ou validação de formulários, o mesmo padrão será utilizado.

\begin{center}
\captionof{sourcecode}{\label{lst:animal-types}Tipos da Camada de Apresentação}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
...
\begin{center}
\captionof{sourcecode}{\label{lst:animal-validations}Esquema de Validação com Zod}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
...
Finalmente, as \textbf{Screens} reúnem todos esses elementos. A tela principal, apresentada no \autoref{lst:animals-screen}, provê o \texttt{AnimalProvider} para toda a árvore de componentes e decide entre exibir a listagem ou os detalhes de um animal específico (sub-abas). Essa padronização arquitetural, separando o contexto, os ganchos, os componentes de \gls{UI} e as regras de negócio de ponta a ponta, foi replicada rigorosamente em todos os módulos do sistema, garantindo uniformidade e alta escalabilidade.

\begin{center}
\captionof{sourcecode}{\label{lst:animals-screen}Renderização Declarativa na Página de Animais}
\end{center}
\vspace{-1.5\baselineskip}
\begin{lstlisting}[language=TypeScript]
